...
scripts/            Scripts √∫tiles (opcional)
```
...
- `services.json`  4 servicios con n√∫meros
- `company.json`  Info de contacto, equipo, certificaciones

Edita JSON si necesitas cambiar datos sin tocar c√≥digo.

### config/

- `colors.json`  Paleta centralizada
- `.env.example`  Template de variables (copiar a `.env`)
- `meta.json`  Metadatos (versi√≥n, autor)

### docs/

Documentaci√≥n:

- `ARCHITECTURE.md`  Por qu√© est√ hecho as√≠
- `CUSTOMIZATION.md`  C√≥mo cambiar cosas
- `DEPLOYMENT.md`  C√≥mo poner en producci√≥n
- `PERFORMANCE.md`  Tips de optimizaci√≥n
- `API-REFERENCE.md`  Qu√© hace cada m√≥dulo JS
- `CHANGELOG.md`  Historial de cambios

### scripts/

Scripts √∫tiles (todos opcionales):
...
| N√∫mero WhatsApp | `src/assets/js/modules/whatsapp-integration.js` |
| Tipograf√≠a | `src/assets/css/variables.css` |
