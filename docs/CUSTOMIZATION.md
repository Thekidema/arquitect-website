...
Aquí está lo que necesitas cambiar para que sea tuyo.

## Colores

Todo está en `config/colors.json`. Si cambias un color allí, se refleja en todo el sitio automáticamente.
...
- `src/assets/images/hero/` - Las 3 imágenes grandes del carrusel
- `src/assets/images/projects/` - Las 6 imágenes del portfolio

Simplemente reemplaza los archivos SVG con tus imágenes (JPG, PNG o WebP). Mantén los nombres iguales.

Si cambias el formato (de .svg a .jpg), actualiza también los `src` en `index.html`.

## Número de WhatsApp

Está en `src/assets/js/modules/whatsapp-integration.js`:
...
**Las imágenes no aparecen**: Revisa que existan en `src/assets/images/` y que los nombres en `index.html` sean exactos.
